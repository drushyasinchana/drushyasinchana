/* ══════════════════════════════════════════════════════
INVOICEEASE PRO - LOGIN PAGE LOGIC
...
window.addEventListener('DOMContentLoaded', () => {
  const cid = document.getElementById('companyId');
...
async function handleLogin() {
  ...
  const err = document.getElementById('loginError');
  const cidIn = document.getElementById('companyId');
  const emlIn = document.getElementById('email');
  ...
  // Basic validation
  if (!companyId) { showError(err, 'Company ID required'); return; }
  if (!email) { showError(err, 'Email address required'); return; }
  if (!password) { showError(err, 'Password required'); return; }
  ...
  try {
    ...
    // 5. Initialize Client Firestore
    const clientConfig = data.firebaseConfig;
    if (!clientConfig?.projectId) {
      throw new Error("Client database configuration missing.");
    }
    
    let clientApp;
    const existing = firebase.apps.find(a => a.name === 'invoiceClient');
    if (existing) clientApp = existing;
    else clientApp = firebase.initializeApp(clientConfig, 'invoiceClient');
    
    const clientDb = clientApp.firestore();
    
    // 6. ✅ STORE COMPANY ID IN SESSION STORAGE (NOT in URL)
    sessionStorage.setItem('currentCompanyId', companyId);
    sessionStorage.setItem('currentUserEmail', email);
    sessionStorage.setItem('invoiceProSession', JSON.stringify({
      companyId: companyId,
    ...
    // 7. ✅ REDIRECT TO CLEAN URL (no query params, no hash)
...
document.addEventListener('keydown', e => {
  if (e.key === 'Enter' && document.getElementById('loginScreen')?.classList.contains('active')) {
    ...
    if (el && ['companyId','email','password'].includes(el.id)) {
      e.preventDefault();
      handleLogin();
    }
  }
