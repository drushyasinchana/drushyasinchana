...
async function openEmpModal() {
  // ✅ Reset Category & Designation dropdowns
  const eCategory = document.getElementById('eCategory');
  if (eCategory) eCategory.value = '';
  
  const eDesignation = document.getElementById('eDesignation');
  if (eDesignation) {
    eDesignation.innerHTML = '<option value="">— Select Category First —</option>';
  }
...
async function loadDesignationsForSector() {
  console.log('🔍 loadDesignationsForSector() - Loading from Firestore...');
  
  const select = document.getElementById('eDesignation');
  if (!select) {
    console.error('❌ #eDesignation element not found');
    return;
  }
  ...
  try {
    // 1. Get company's sector code from settings
    ...
    if (!settingsDoc.exists) {
      console.warn('⚠️ Settings doc not found, using fallback');
      loadFallbackDesignations(select);
      return;
    }
    
    const settingsData = settingsDoc.data();
    const sectorCode = settingsData.sectorCode;
    
    if (!sectorCode) {
      console.warn('⚠️ No sectorCode in settings, using fallback');
      loadFallbackDesignations(select);
      return;
    }
    
    console.log(`  ✓ Company sector: ${sectorCode}`);
    
    // 2. Fetch sector document from sectors collection
    const sectorDoc = await S.clientDb.collection('sectors').doc(sectorCode).get();
    
    if (!sectorDoc.exists) {
      console.error(`❌ Sector "${sectorCode}" not found in sectors collection`);
      loadFallbackDesignations(select);
      return;
    }
    
    const sectorData = sectorDoc.data();
    const designations = sectorData.designations;
    
    if (!designations) {
      console.error('❌ No designations field in sector document');
      loadFallbackDesignations(select);
      return;
    }
    
    // 3. Populate dropdown with optgroups (Office/Field categories)
    let totalCount = 0;
    ...
    console.log(`✅ Loaded ${totalCount} designations from sectors/${sectorCode}`);
    
  } catch (e) {
    console.error('❌ Failed to load designations from Firestore:', e);
    loadFallbackDesignations(select);
  }
}

// Fallback function if Firestore load fails
function loadFallbackDesignations(select) {
  console.log('  ⚠️ Using fallback designations');
  const fallback = [
    "Manager", "Supervisor", "Foreman", "Engineer", 
    "Technician", "Worker", "Security", "Driver", "Admin"
  ];
...
function editEmployee(e) {
  ...
  // 2. Disable Employee ID field
  const eCode = document.getElementById('eCode');
  if (eCode) eCode.disabled = true;

  // 3. ✅ FIX: Convert dates to YYYY-MM-DD format for input type="date"
  ...
  const eJoin = document.getElementById('eJoin');
  const eEffDate = document.getElementById('eEffDate');
  if (eJoin) eJoin.value = toDateInput(e.JoinDate);
  if (eEffDate) eEffDate.value = toDateInput(e.EffectiveDate);

  // 4. Set other select fields
  const eStatus = document.getElementById('eStatus');
  const eRole = document.getElementById('eRole');
  if (eStatus) eStatus.value = e.Status || 'ACTIVE';
  if (eRole) eRole.value = e.Role || 'EMPLOYEE';

  // 5. ✅ FIX: Populate sites FIRST, then set value with delay
  populateSiteSelects();
  
  // Small delay to ensure options are loaded before setting value
  setTimeout(() => {
    const eSite = document.getElementById('eSite');
    if (eSite && e.Site) {
      eSite.value = e.Site;
      console.log('Site set to:', e.Site);
    }
  }, 100);

  // 6. Display photo in modal
    ...
    if (e.Photo) {
      try {
        ...
        if (typeof e.Photo.toBytes === 'function') {
          ...
          imgUrl = e.Photo.startsWith('data:') ? e.Photo : `image/jpeg;base64,${e.Photo}`;
        }
